...
TOO_BROAD = ["식품","농산물","축산물","수산물","과일","채소","정육","건어물",
             "가공식품","신선식품","farm","food"]
CATEGORY_MAP = {"과일":"과일/농산물","농산물":"과일/농산물","채소":"과일/농산물",
...
BUY_CAT = {"과일/농산물": ["송이","알","브릭스","당도","제철","가정용","못난이","정품","특상","특대"],
...
if "related_info" not in st.session_state:
    st.session_state.related_info = ""
...
with topbar:
    ...
    if st.session_state.selected:
        ...
        for start in range(0, len(kws), per_row):
            row = kws[start:start+per_row]
            cols = st.columns(per_row)
            for col, kw in zip(cols, row):
                col.button(f"✕ {kw}", key=f"del_{kw}",
                           on_click=remove_keyword, args=(kw,),
                           use_container_width=True)
        c1, c2 = st.columns([1, 5])
        c1.button("비우기", on_click=clear_all)
        c2.code(",".join(st.session_state.selected) + ",")
    else:
        st.caption("아직 담은 키워드가 없어요. 아래 목록의 ➕ 를 눌러 담아보세요.")
...
if st.session_state.results:
    st.info("자동 인식된 상위어: " + st.session_state.related_info)
